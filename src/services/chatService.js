...
class ChatService {
  ...
  async getChatResponse(message) {
    ...
    const requestBody = {
      message,
      email: 'nag@example.com', // Default email for now
      mode: 'Author' // Default mode
    };
    ...
    try {
      ...
      if (response.status === 502) {
        console.log('Received 502 error, retrying...');
      ...
      try {
        ...
        // If we can't parse the response, try to get a new response
        console.log('Retrying due to parse error...');
        await new Promise(resolve => setTimeout(resolve, 2000));
        return this.getChatResponse(message);
      }

      console.log('Chat response data:', JSON.stringify(responseData, null, 2));

      if (!responseData || !responseData.response) {
        throw new Error(AZURE_API_ERRORS.INVALID_RESPONSE);
      }

      const audioUrl = responseData.audio_url;
      console.log('Returning response with audio_url:', audioUrl);
